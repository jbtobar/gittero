...
function calculateTotals() {
	...
	for (var i = 0; i < tb["_groups"][0][0].children.length; i++) {
		 jon = jon + Number(tb["_groups"][0][0].children[i].attributes.value.value)
	}
	console.log(jon)
	d3.select('#mr_sum').text(jon.toFixed(0))



	var tb = d3.select('#order_tbody')
	var jon = 0
	for (var i = 0; i < tb["_groups"][0][0].children.length; i++) {
		 jon = jon + Number(tb["_groups"][0][0].children[i].attributes.value.value)
	...
	d3.select('#dc_sum').text(jon.toFixed(0))
...
function connectToTable(aro,otype) {
	...
	row.append('input').attr('id',coda+'_order_qty').attr('class','qty_input').attr('type','number').attr('value',10).attr('onchange','updateTotals(this)')
	...
	var bgop = tots.append('td').attr('id',code+'_bgop').attr('value',Number(margins[code]["bgop"])).text((Number(margins[code]["bgop"]) * qty).toFixed(0))
	var bgonp = tots.append('td').attr('id',code+'_bgonp').attr('value',Number(margins[code]["bgonp"])).text((Number(margins[code]["bgonp"]) * qty).toFixed(0))
	var buydepo = tots.append('td').attr('id',code+'_buydepo').attr('value',Number(margins[code]["buydepo"])).text((Number(margins[code]["buydepo"]) * qty).toFixed(0))
	var go 
	if (side == 'Buy') {
	...
	gtkrow.append('td').attr('value',Number(delt)).text(Number(delt).toFixed(2))
	gamm = d3.select('#'+code+'_gamma')["_groups"][0][0].attributes.value.value
	gtkrow.append('td').attr('value',Number(gamm)).text(Number(gamm).toFixed(2))
	thet = d3.select('#'+code+'_theta')["_groups"][0][0].attributes.value.value
	gtkrow.append('td').attr('value',Number(thet)).text(Number(thet).toFixed(0))
	veg = d3.select('#'+code+'_vega')["_groups"][0][0].attributes.value.value
	gtkrow.append('td').attr('value',Number(veg)).text(Number(veg).toFixed(0))
...
function shrinkToggle(smth) {
	...
	if (smth.id == 'mid_pane_header') {
		if (ind == 1) {
		var o = 250
		var width_or_height = 'height'
		var section = 'mid_pane_header'
		shrinkOn()
...
//
// THIS IS ANOTHER SESSION, DEALING WITH RTS_MIN RVI_MIN
//
